...
// Security handlers – new ones
import { handleAntiCaps } from '../plugins/anticaps.js';
import { handleAntiEmoji } from '../plugins/antiemoji.js';
import { handleAntiGif } from '../plugins/antigif.js';
import { handleAntiVoice } from '../plugins/antivoice.js';
import { handleAntiEmail } from '../plugins/antiemail.js';
import { handleAntiIp } from '../plugins/antiip.js';
import { handleAntiFlood } from '../plugins/antiflood.js';
import { handleAntiMassMention } from '../plugins/antimassmention.js';
import { handleAntiUrl } from '../plugins/antiurl.js';
import { handleAntiMedia } from '../plugins/antimedia.js';
import { handleAntiSticker } from '../plugins/antisticker.js';
import { handleAntiLocation } from '../plugins/antilocation.js';
import { handleAntiContact } from '../plugins/anticontact.js';
import { handleAntiPoll } from '../plugins/antipoll.js';
import { handleAntiForward } from '../plugins/antiforward.js';
import { handleAntiPhone } from '../plugins/antiphone.js';
import { handleSlowMode } from '../plugins/slowmode.js';
import { handleTempMute } from '../plugins/tempmute.js';
...
async function handleMessages(sock, messageUpdate) {
    try {
        ...
        // Store pushName in contacts for name resolution
        ...
        // ========== AUTO‑REPLY (only if not a command) ==========
        if (!message.key.fromMe && !userMessage.startsWith(config.prefixes[0])) {
        ...
        // ========== COMMAND DETECTION (MUST BE BEFORE OTHER SECURITY) ==========
        ...
        // If it's a command, execute it immediately (bypass security filters)
        if (command) {
            const senderIsOwnerOrSudo = await isOwnerOrSudo(senderId, sock, chatId);
            const isOwnerOrSudoCheck = message.key.fromMe || senderIsOwnerOrSudo;
        ...
        // ========== NO COMMAND – RUN SECURITY CHECKS AND OTHER FEATURES ==========
        const senderIsOwnerOrSudo = await isOwnerOrSudo(senderId, sock, chatId);
        const isOwnerOrSudoCheck = message.key.fromMe || senderIsOwnerOrSudo;
        ...
        // Anti‑spam flood detection (only in groups)
        ...
        if (!isGroup && !message.key.fromMe && !(await isSudo(senderId))) {
            try {
        ...
        // ========== OTHER SECURITY HANDLERS (run only if no command) ==========
        if (await handleTempMute(sock, chatId, message, userMessage, senderId)) return;
        if (await handleSlowMode(sock, chatId, message, userMessage, senderId)) return;
        if (await handleAntiFlood(sock, chatId, message, senderId)) return;
        if (await handleAntiGif(sock, chatId, message, senderId)) return;
        if (await handleAntiVoice(sock, chatId, message, senderId)) return;
        if (await handleAntiEmail(sock, chatId, message, userMessage, senderId)) return;
        if (await handleAntiIp(sock, chatId, message, userMessage, senderId)) return;
        if (await handleAntiMassMention(sock, chatId, message, senderId)) return;
        if (await handleAntiUrl(sock, chatId, message, userMessage, senderId)) return;
        if (await handleAntiMedia(sock, chatId, message, senderId)) return;
        if (await handleAntiSticker(sock, chatId, message, senderId)) return;
        if (await handleAntiLocation(sock, chatId, message, senderId)) return;
        if (await handleAntiContact(sock, chatId, message, senderId)) return;
        if (await handleAntiPoll(sock, chatId, message, senderId)) return;
        if (await handleAntiForward(sock, chatId, message, userMessage, senderId)) return;
        if (await handleAntiPhone(sock, chatId, message, userMessage, senderId)) return;
        if (await handleAntiCaps(sock, chatId, message, userMessage, senderId)) return;
        if (await handleAntiEmoji(sock, chatId, message, userMessage, senderId)) return;

        // ========== AUTO‑TYPING ==========
        await handleAutotypingForMessage(sock, chatId, userMessage);

        // ========== GROUP‑ONLY FEATURES (tag detection, mentions, chatbot) ==========
        if (isGroup) {
